...
class StockReportController extends CoreController
{
    ...
    public function stockOverviewReportData(Request $request)
    {
        ...
        $defaultwar=$request->defaultwar;
        $stockQuantity=StockManagement::where('stock_item_id',$itemname)
                        ->with(['stockitem','batch','warehouse'])
                        ->where('active',1)
                        ->selectRaw('id,stock_item_id,batch_id,count(document_no) as document_number,warehouse_id,sum(qty) as quantity,voucher_type')
                        ->groupBy('stock_item_id','warehouse_id')
                        ->get();
        $receiptQty=StockIssue::where('stock_item_id',$itemname)
                                ->selectRaw('sum(remaining_qty) as remainingqty')
                                ->groupBy('stock_item_id')
                                ->first();
        
        return json_encode(['stockQuantity'=>$stockQuantity,'receiptQty'=>$receiptQty ?? 0]);
    }
}
