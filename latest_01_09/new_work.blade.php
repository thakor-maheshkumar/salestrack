...
<div class="form-group">
    ...
    {!! Form::label('branch_id', 'Branch') !!}
            {!! Form::select('branch_id', ['' => 'Select a Branch'] + $consignee_address, old('branch_id', isset($order->supplier_id) ? $order->branch_id : ''), ['class' => 'form-control', (count($consignee_address) == 0) ? 'disabled' : ''] ) !!}
...
//                console.log(po_id);
                ...
                $.ajax({
                    ...
                    success:function(data){
                            ...
                            branch_id.empty();
                            $.each(data.branches, function(index3, value3) {
                                branch_id.append($("<option />").val(index3).text(value3));
                            });*/
                            //console.log(data.po_order_details);
                            ...
                            var mahi=data.po_order_details.supplier_id;
                            $.ajax({
                                type:"POST",
                                url:"{{url('admin/transactions/purchase/getReceiptDetails')}}",
                                data:{mahi:mahi},
                                success:function(data){
                                    console.log(data);
                                    //console.log(data);
                                    //$('#branch_id').val(data.supplier_details.branch_name);
                                    /*var consineeAdd = (data.supplier_details.branch_name);
                                   var consineeAdd = data.supplier_details.branch_name
                                  ...
                                  /*    var branch_id = $('#branch_id');
                                       branch_id.empty();*/
                                       var options='';
                                        $.each(data.supplier_details.branch_name, function(key, obj) {
                                        //branch_id.append($("<option />").val(index3).text(value3));
                                        options += '<option value="'+data.supplier_details.branch_name+'">' +data.supplier_details.branch_name+ '</option>';   
                                       });
                                        $('#branch_id').html(options);
                                }
                            })
