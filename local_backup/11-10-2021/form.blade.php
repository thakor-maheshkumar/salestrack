            ...
            @else
                <tr class="new-item">
                   ...
                   {{--  <td>
                    ...
                    <td>
                        {!! Form::select("source_items[$element_counter][target_batch]", ['' => 'Select Batch'] + $batches, ['class' => 'form-control tbatch select2-elem', 'data-name' => 'batch']) !!}
                    </td>
                    <td>
                        <div class="delete-item-block">
                            <a href="javascript:void(0);"
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#default_source_warehouse').change(function(){
      
});
        
        $(document).on('change', '#default_target_warehouse', async function() {
            var selected = $(this).val();
            //alert(selected);
            ...
            if((item_count == 1) && selected)
            {
                $(".new-item .target_warehouse").val(selected);
            }

            // New Coding Here Today 07-10-2021
            ...
            var pk=$('add-more-item').closest('tr');
            var masi=$(this).find("td.add-more-value").val();
            //var add_more_value=(add_more_item.find('.maheshItem'+element_mk).val());
            //var input=[];
            ...
            await resolveAfter2Seconds(data, i);
            }           
           /* alert(jk)*/;
        });
                ...
                success:function(response){
                    ...
                    var tableData='<tr class="new-item add-more-item">'+
                        ...
                        '<td>'+'<select name="source_items['+element_counter+'][source_warehouse]" class="form-control warehousesMahesh" >'+'<option value="'+value.source_warehouse+'">'+value.warehouse.name+'</option>'+'</select>'+'</td>'+
                                    ...
                                    '<td class="add-more-value">'+'<select name="source_items['+element_counter+'][item_name]" class="form-control item_name maheshItem">'+'<option value="'+value.item_id+'">'+value.item_name+'</option>'+'</select>'+'</td>'+
                        ...
                        '<td>'+'{!! Form::select("source_items[$element_counter][target_warehouse]", ['' => "Select Warehouse"] + $warehouses, null, ["class" => "form-control default_target_warehouse target_warehouse", "data-name" => "target_warehouse"]) !!}'+'</td>'+
                                    /*
                                    '<td>'+'<select name="source_items['+element_counter+'][target_warehouse]" class="form-control target_warehouse warehouse_select default_target_warehouse" >'+'<option value="'+value.source_warehouse+'">'+value.warehouse.name+'</option>'+'</select>'+'</td>'+*/
                                     ...
                                     '<td class="mka">'+'<select class="form-control batch target_mahesh_batch" name=" source_items['+element_counter+'][target_batch]"></select>'+'</td>'+ 
