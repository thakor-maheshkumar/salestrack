    ...
    <div class="form-row">
        <div class="col-md-12">
           <!--  <label>Purchase Receipt Series</label>
            ...
            @if($order->receipt_no!='')
                ...
                <input type="text" class="form-control" name="receipt_no" value="{{$order->receipt_no}}">
             @else
        ...
        <div class="col-md-12">
           ...
           {!! Form::text('receipt_no', old('receipt_no', isset($order->receipt_no) ? $order->receipt_no : ''), ['class' => 'form-control series_id', 'id' => 'series_id','readonly' ]) !!}
            ...
            {!! Form::label('po_id', 'PO Reference') !!}
            {!! Form::select('po_id', ['0' => 'Select a PO'] + $purchase_orders , old('po_id', isset($order->po_id) ? $order->po_id : $order->id ?? ''), ['class' => 'form-control po_id','required' => 'required'] ) !!}
...
<div class="form-group">
    ...
    <div class="col-md-12">
            ...
            {!! Form::select('branch_id', [$order->main_branch => isset($order->main_branch)? $order->main_branch :'Select a Branch'] + $consignee_address, old('branch_id', isset($order->supplier_id) ? $order->branch_id : ''), ['class' => 'form-control branch_id', (count($consignee_address) == 0) ? 'disabled' : ''] ) !!}
...
<fieldset class="hidden-block scheduler-border">
        ...
        <div class="form-row">
            <div class="col-md-12">
                ...
                <select id="stock_item_id" name="stock_item_id" data-plugin-selectTwo class="form-control stock_item_id select2-elem select2" data-name="stock_item_id" required="required">
                    ...
                    <option value="{{ $value->id }}" @if(isset($order->items->stock_item_id)) @if($order->items->stock_item_id == $value->id){{"selected='selected'"}} @endif @elseif(isset($order->purchase_items->stock_item_id)) @if($order->purchase_items->stock_item_id == $value->id){{"selected='selected'"}} @endif @endif> {{ $value->pack_code.'-'.$value->name }} </option>
                    @endforeach
                    @endif
                </select>
    ...
    <div class="form-group">
        <div class="form-row">
            <div class="col-md-12">
                {!! Form::label('unit', 'UOM') !!}
                {!! Form::select("unit", $units, old('unit', isset($order->unit) ? $order->unit : $order->purchase_items->unit ?? ''), ['class' => 'form-control']) !!}
            </div>
            <div class="col-md-12">
                {!! Form::label('po_quantity', 'PO Quantity *') !!}
                {!! Form::text('po_quantity', old('po_quantity', isset($order->purchase_items->quantity) ? $order->purchase_items->quantity : $order->purchase_items->po_quantity ?? ''), ['class' => 'form-control po_quantity','placeholder' => 'PO Quantity','required' => 'required']) !!}
            </div>
        </div>
    </div>
    <div class="form-group">
...
//                console.log(po_id);
                ...
                $.ajax({
                    ...
                    success:function(data){
                            ...
                            $('#warehouse_id').val(data.po_order_details.warehouse_id)
