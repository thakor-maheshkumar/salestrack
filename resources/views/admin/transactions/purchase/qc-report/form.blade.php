...
<div class="form-group">
    <div class="form-row">
        <div class="col-md-12">
           ...
           {{--  {!! Form::select('grade_id', ['' => 'Select a Grade'] + $stock_grade, old('batch_id', isset($item->batch_id) ? $item->batch_id : ''), ['onchange'=>'getBatchDetails(this.value)','class' => 'form-control','required'=>'required'] ) !!} --}}
        ...
        {{-- <div class="col-md-12">
            <div class="form-group">
                ...
                {!! Form::select('batch_id', ['' => 'Select a Batch'] + $batch, old('batch_id', isset($item->batch_id) ? $item->batch_id : ''), ['onchange'=>'getBatchDetails(this.value)','class' => 'form-control','required'=>'required'] ) !!}
            </div>
        </div> --}}
    </div>
</div>
<div class="form-group">
    <div class="form-row">
        <div class="col-md-12">
            ...
            {!! Form::text('receipt_no', old('receipt_no', isset($item->receipt_no) ? $item->receipt_no : ''), ['class' => 'form-control', 'placeholder' => 'Receipt No','readonly' => 'readonly']) !!}
        </div>
        <div class="col-md-12">
...
<div class="form-group">
    <div class="form-row">
        <div class="col-md-12">
            ...
            {!! Form::text('batch_size', '' , ['class' => 'form-control batch_size', 'placeholder' => 'Batch Size','disabled' => 'disabled']) !!}
        </div>
        <div class="col-md-12">
            <div class="form-group">
    ...
    <script type="text/javascript">
            ...
            if(grade_id != '')
            {
                ...
                $.ajax({
                    ...
                    success:function(data){
                        $('.add-items-list').html(data.qc_tests);
                    }
                });
            }
        }
        function getBatchDetails(batch_id)
        {
            if(batch_id != '')
            {
                ...
                $.ajax({
                    type:'POST',
                    url:'{{url("admin/transactions/purchase/getBatchDetails")}}',
                    data:{batch_id:batch_id},
                    success:function(data){
                        console.log(data);
                        $('.batch_size').val(data.batch.batch_size);
                        $('.manufacturing_date').val(data.batch.manufacturing_date);
                    }
                });
            }
