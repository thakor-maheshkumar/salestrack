...
<body>
    <section class="pdf-block">
        ...
        <table border="1" class="main-table" style="width:670px">
            <tbody>
               <!--  <tr>
                ...
                <tr>
                    <td colspan="7" align="center"><b>Purchase Return</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Return No :  {{ isset($purchaseReturn->return_no) ? $purchaseReturn->return_no : '' }}</b></p>
                        <p><b>Invoice No :  {{ isset($purchaseReturn->invoice->invoice_no) ? $purchaseReturn->invoice->invoice_no : '' }}</b></p>
                        <p><b>Order Date: {{ isset($purchaseReturn->return_date) ? $purchaseReturn->return_date : '' }}</b></p>
                        <p><b>Supplier Name: {{ isset($purchaseReturn->suppliers->ledger_name) ? $purchaseReturn->suppliers->ledger_name : '' }}</b></p>
                        <p><b>Approved Vendor Code: {{$purchaseReturn->approved_vendor_code}}</b></p>
                        <p><b>Branch Name: {!! (isset($purchaseReturn->branches)) ? $purchaseReturn->branches->branch_name : '-' !!}</b></p>
                        <p><b>Address: {!! (isset($purchaseReturn->address)) ? $purchaseReturn->address : '-' !!}</b></p>
                ...
                @if(isset($purchaseReturn->items) && $purchaseReturn->items->isNotEmpty())
                    @php
                        $i = 1;
                    @endphp
                    @foreach($purchaseReturn->items as $key => $item)
                        <tr>
                            ...
                            <td>{{$purchaseReturn->required_date}}</td>
                ...
                <tr>
                    ...
                    @php
                    $total=((int)$item->net_amount -(int)$purchaseReturn->items->get(0)->discount);
                    @endphp
                    <td align="right">{{ isset($purchaseReturn->items->get(0)->discount_in_per) ? $purchaseReturn->items->get(0)->discount_in_per : '0.00' }}%</td>
                    <td align="right">{{$purchaseReturn->items->get(0)->discount}}</td>
                </tr>
                <tr>
                ...
                <tr>
                    @php
                    $sgst=((int)$purchaseReturn->items->get(0)->tax/2)
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">SGST</td>
                    @foreach($state as $key=>$value)
                    @if($value->state==$purchaseReturn->state)
                    ...
                    <td align="right">{{ isset($purchaseReturn->sgst) ? $purchaseReturn->sgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    @endif
                    @endforeach
                    <td align="right">{{ isset($purchaseReturn->cgst) ? $purchaseReturn->cgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    <td align="right">0</td>
                    @else
                    <td align="right">{{$purchaseReturn->items->get(0)->tax}}%</td>
                    @endif
                    @endforeach
                    
                    <td align="right">{{ isset($purchaseReturn->igst) ? $purchaseReturn->igst : '0.00' }}.00</td>
                </tr>
                <tr>
                     @php
                        $total_amt=((int)$purchaseReturn->items->get(0)->tax_amount + (int)$purchaseReturn->total_net_amount);
                     @endphp
                ...
                <tr>
                    ...
                    <td align="right">{{$purchaseReturn->total_other_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"><b>Credit Days: {{ isset($purchaseReturn->credit_days) ? $purchaseReturn->credit_days : 0 }} Days PDC</b></td>
                    <td colspan="4" align="right"><b>TOTAL GROSS AMOUNT (Rs.)</b></td>
                    <td align="right"><b>{{ $total_amt + (int)$purchaseReturn->total_other_net_amount}}</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Delivery : at {{ isset($purchaseReturn->target_warehouse->name) ? ' ' . $purchaseReturn->target_warehouse->name : '' }}</b></p>
                        <p><b>Transpoter : {{ isset($purchaseReturn->transporter) ? $purchaseReturn->transporter : '' }}</b></p>
                        <p><b>Reference  : {{ isset($purchaseReturn->reference) ? $purchaseReturn->reference : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
