                ...
                <label for="order_no">Material Request Number *</label>
                    ...
                    <option value="">Select Material ID</option>
                        @foreach($materials_data as $value)
                            ...
                            @endphp
                           <option value="{{ $value->id }}"   @if(isset($order->material_id)) @if($order->material_id == $value->id){{"selected='selected'"}} @endif @elseif(isset($order->id))@if($order->id == $value->id){{"selected='selected'"}} @endif @endif > {{ $value->series_id . ((!empty($item_name)) ? ' (' . $item_name . ')' : '') }} </option>
                        @endforeach
