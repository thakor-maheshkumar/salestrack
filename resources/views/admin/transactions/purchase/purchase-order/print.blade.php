    ...
    <section class="pdf-block">
        ...
        <table border="1" class="main-table" style="width:680px">
            <tbody>
                ...
                <tr>
                    <td colspan="7">
                        <p><b>PO Ref  No :  {{ isset($purchaseOrder->order_no) ? $purchaseOrder->order_no : '' }}</b></p>
                        <p><b>Order Date: {{ isset($purchaseOrder->order_date) ? $purchaseOrder->order_date : '' }}</b></p>
                        <p><b>Supplier Name: {{ isset($purchaseOrder->suppliers->ledger_name) ? $purchaseOrder->suppliers->ledger_name : '' }}</b></p>
                        <p><b>Approved Vendor Code: {{$purchaseOrder->approved_vendor_code}}</b></p>
                        <p><b>Branch Name: {!! (isset($purchaseOrder->branches)) ? $purchaseOrder->branches->branch_name : $purchaseOrder->main_branch!!}</b></p>
                        <p><b>Address: {!! (isset($purchaseOrder->address)) ? $purchaseOrder->address : '-' !!}</b></p>
                ...
                @if(isset($purchaseOrder->items) && $purchaseOrder->items->isNotEmpty())
                    @php
                        $i = 1;
                    @endphp
                ...
                <tr>
                    ...
                    @php
                   $remain = ((int)$item->net_amount - (int)$purchaseOrder->items->get(0)->discount);
                    @endphp
                    <!-- $remain = ((int)$item->net_amount - (int)$purchaseOrder->items->get(0)->discount); -->

                    <td align="right">{{ isset($purchaseOrder->items->get(0)->discount_in_per) ? $purchaseOrder->items->get(0)->discount_in_per : '0.00' }}%</td>
                    <td align="right">{{$purchaseOrder->items->get(0)->discount}}</td>
                </tr>
                <tr>
                ...
                <tr>@php
                    $sgst=((int)$purchaseOrder->items->get(0)->tax/2)
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">SGST</td>
                    @foreach(\App\Models\Company::all() as $companystate)
                    @if($companystate->state==$purchaseOrder->state)
                    <td align="right">{{ $sgst}}%</td>
                    @else
                    <td align="right">0%</td>
                    @endif
                    @endforeach
                    <td align="right">{{ isset($purchaseOrder->sgst) ? $purchaseOrder->sgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    @endif
                    @endforeach
                    <td align="right">{{isset($purchaseOrder->cgst) ? $purchaseOrder->cgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    <td align="right">0%</td>
                    @else
                    <td align="right">{{$purchaseOrder->items->get(0)->tax}}%</td>
                    @endif
                    @endforeach
                    <td align="right">{{ isset($purchaseOrder->igst) ? $purchaseOrder->igst : '0.00' }}.00</td>
                </tr>
                <tr>
                    <td colspan="6" align="right">TOTAL AMOUNT</td>
                    <td align="right">{{ $purchaseOrder->total_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"></td>
                    <td colspan="4" align="right">FREIGHT</td>
                    <td align="right">{{$purchaseOrder->total_other_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"><b>Credit Days: {{ isset($purchaseOrder->credit_days) ? $purchaseOrder->credit_days : 0 }} Days PDC</b></td>
                    <td colspan="4" align="right"><b>TOTAL GROSS AMOUNT (Rs.)</b></td>
                    <td align="right"><b>{{ isset($purchaseOrder->grand_total) ? $purchaseOrder->grand_total : '0.00' }}</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Delivery : at {{ isset($purchaseOrder->target_warehouse->name) ? ' ' . $purchaseOrder->target_warehouse->name : '' }}</b></p>
                        <p><b>Transpoter : {{ isset($purchaseOrder->transporter) ? $purchaseOrder->transporter : '' }}</b></p>
                        <p><b>Reference  : {{ isset($purchaseOrder->reference) ? $purchaseOrder->reference : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
