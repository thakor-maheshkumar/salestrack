    ...
    <section class="pdf-block">
        ...
        <table border="1" class="main-table">
            <tbody>
                ...
                <tr>
                    <td colspan="7" align="center"><b>Delivery Note</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Order No :  {{ isset($deliveryNote->delivery_no) ? $deliveryNote->delivery_no : '' }}</b></p>
                        <p><b>Order No :  {{ isset($deliveryNote->sales_order->order_no) ? $deliveryNote->sales_order->order_no : '' }}</b></p>
                        <p><b>Required Date: {{ isset($deliveryNote->required_date) ? $deliveryNote->required_date : '' }}</b></p>
                        <p><b>Customer Name: {{ isset($deliveryNote->customers->ledger_name) ? $deliveryNote->customers->ledger_name : '' }}</b></p>
                        <p><b>Branch Name: {!! (isset($deliveryNote->branch)) ? $deliveryNote->branch->branch_name : '-' !!}</b></p>
                        <p><b>Approved Vendor Code: {!! (isset($deliveryNote->approved_vendor_code)) ? $deliveryNote->approved_vendor_code : '-' !!}</b></p>
                        <p><b>Address: {!! (isset($deliveryNote->address)) ? $deliveryNote->address : '-' !!}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
                ...
                @if(isset($deliveryNote->items) && $deliveryNote->items->isNotEmpty())
                    @php
                        $i = 1;
                    @endphp
                    @foreach($deliveryNote->items as $key => $item)
                        <tr>
                            <td align="center">{{ $i }}</td>
                            <td>{{ isset($item->stockItems->name) ? $item->stockItems->name : '' }}</td>
                            <td>{{$deliveryNote->required_date}}</td>
                            <td>{{ isset($item->quantity) ? $item->quantity : '' }}</td>
                            <td>{{ isset($item->unit) ? $item->unit : '' }}</td>
                            <td>{{ isset($item->rate) ? $item->rate : '' }}</td>
                            <td align="right">{{ isset($deliveryNote->items->get(0)->net_amount) ? $item->net_amount : '0.00' }}</td>
                        </tr>
                        @php
                            $i++;
                        @endphp
                ...
                <tr>
                    @php
                    $total=((int)$deliveryNote->items->get(0)->net_amount -(int)$deliveryNote->items->get(0)->discount);
                    @endphp
                    <td colspan="3" align="right" class="no-border-b no-border-t">DISCOUNT</td>
                    <td align="right">{{ isset($deliveryNote->items->get(0)->discount_in_per) ? $deliveryNote->items->get(0)->discount_in_per : '0.00' }}%</td>
                    <td align="right">{{$deliveryNote->items->get(0)->discount}}</td>
                </tr>
                <tr>
                ...
                <tr>
                    @php
                    $sgst=((int)$deliveryNote->items->get(0)->tax/2);
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">SGST</td>
                    @foreach($state as $key=>$value)
                    @if($deliveryNote->state==$value->state)
                    <td align="right">{{$sgst}}%</td>
                    @else
                    <td align="right">0%</td>
                    @endif
                    @endforeach
                    <td align="right">{{ isset($deliveryNote->sgst) ? $deliveryNote->sgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    @endif
                    @endforeach
                    <td align="right">{{ isset($deliveryNote->cgst) ? $deliveryNote->cgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    <td align="right">0%</td>
                    @else
                    <td align="right">{{$deliveryNote->items->get(0)->tax}}%</td>
                    @endif
                    @endforeach
                    
                    <td align="right">{{ isset($deliveryNote->igst) ? $deliveryNote->igst : '0.00' }}.00</td>
                </tr>
                <tr>
                    <td colspan="6" align="right">TOTAL AMOUNT</td>
                    <td align="right">{{$deliveryNote->total_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"></td>
                    <td colspan="4" align="right">FREIGHT</td>
                    <td align="right">{{$deliveryNote->total_other_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"><b>Credit Days: {{ isset($deliveryNote->credit_days) ? $deliveryNote->credit_days : 0 }} Days PDC</b></td>
                    <td colspan="4" align="right"><b>TOTAL GROSS AMOUNT (Rs.)</b></td>
                    <td align="right"><b>{{ isset($deliveryNote->grand_total) ? $deliveryNote->grand_total : '0.00' }}</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Delivery : at {{ isset($deliveryNote->target_warehouse->name) ? ' ' . $deliveryNote->target_warehouse->name : '' }}</b></p>
                        <p><b>Transpoter : {{ isset($deliveryNote->transporter) ? $deliveryNote->transporter : '' }}</b></p>
                        <p><b>Reference  : {{ isset($deliveryNote->reference) ? $deliveryNote->reference : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
