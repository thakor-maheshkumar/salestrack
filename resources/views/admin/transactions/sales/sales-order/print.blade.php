    ...
    <section class="pdf-block">
        ...
        <table border="1" class="main-table">
            <tbody>
                ...
                <tr>
                    <td colspan="7" align="center"><b>Sales Order</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>PO Ref  No :  {{ isset($salesOrder->order_no) ? $salesOrder->order_no : '' }}</b></p>
                        <p><b>Order Date: {{ isset($salesOrder->order_date) ? $salesOrder->order_date : '' }}</b></p>
                        <p><b>Customer Name: {{ isset($salesOrder->customers->ledger_name) ? $salesOrder->customers->ledger_name : '' }}</b></p>
                        <p><b>Sales Person: {{ isset($salesOrder->salesPerson->name) ? $salesOrder->salesPerson->name : '' }}</b></p>
                        <p><b>Branch Name: {!! (isset($salesOrder->branch)) ? $salesOrder->branch->branch_name : '-' !!}</b></p>
                        <p><b>Quotation No: {{ isset($salesOrder->quotation->quotation_no) ? $salesOrder->quotation->quotation_no : '' }}</b></p>
                ...
                <tr>
                    <td colspan="7">
                        ...
                        <p>{{$salesOrder->state}} 400 009.  (Ph.){{$salesOrder->customers->landline_no}}</p>
                    </td>
                </tr>
                <tr>
                ...
                @if(isset($salesOrder->items) && $salesOrder->items->isNotEmpty())
                    @php
                        $i = 1;
                    @endphp
                    @foreach($salesOrder->items as $key => $item)
                        <tr>
                            ...
                            <td>{{$salesOrder->required_date}}</td>
                ...
                <tr>
                    ...
                    $total = ((int)$item->net_amount - (int)$salesOrder->items->get(0)->discount);
                    
                    @endphp
                    <td align="right">{{ isset($salesOrder->items->get(0)->discount_in_per) ? $salesOrder->items->get(0)->discount_in_per : '0.00' }}%</td>
                    <td align="right">{{$salesOrder->items->get(0)->discount}}</td>
                </tr>
                <tr>
                ...
                <tr>
                    @php
                    $sgst=((int)$salesOrder->items->get(0)->tax/2);
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">SGST</td>
                    @foreach($state as $key=>$value)
                    @if($salesOrder->state==$value->state)
                    ...
                    <td align="right">{{ isset($salesOrder->sgst) ? $salesOrder->sgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    @endif
                    @endforeach
                    <td align="right">{{ isset($salesOrder->cgst) ? $salesOrder->cgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    <td align="right">0%</td>
                    @else
                    <td align="right">{{$salesOrder->items->get(0)->tax}}%</td>
                    @endif
                    @endforeach
                    <td align="right">{{ isset($salesOrder->igst) ? $salesOrder->igst : '0.00' }}.00</td>
                </tr>
                <tr>
                    <td colspan="6" align="right">TOTAL AMOUNT</td>
                    <td align="right">{{$salesOrder->total_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"></td>
                    <td colspan="4" align="right">FREIGHT</td>
                    <td align="right">{{ isset($salesOrder->total_other_net_amount) ? $salesOrder->total_other_net_amount : '0.00' }}</td>
                </tr>
                <tr>
                    <td colspan="2"><b>Credit Days: {{ isset($salesOrder->credit_days) ? $salesOrder->credit_days : 0 }} Days PDC</b></td>
                    <td colspan="4" align="right"><b>TOTAL GROSS AMOUNT (Rs.)</b></td>
                    <td align="right"><b>{{ isset($salesOrder->grand_total) ? $salesOrder->grand_total : '0.00' }}</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Delivery : at {{ isset($salesOrder->target_warehouse->name) ? ' ' . $salesOrder->target_warehouse->name : '' }}</b></p>
                        <p><b>Transpoter : {{ isset($salesOrder->transporter) ? $salesOrder->transporter : '' }}</b></p>
                        <p><b>Reference  : {{ isset($salesOrder->reference) ? $salesOrder->reference : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
