    ...
    <section class="pdf-block">
        ...
        <table border="1" class="main-table">
            <tbody>
                ...
                <tr>
                    <td colspan="7" align="center"><b>Sales Invoice</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Invoice No :  {{ isset($salesInvoice->invoice_no) ? $salesInvoice->invoice_no : '' }}</b></p>
                        <p><b>Date: {{ isset($salesInvoice->invoice_date) ? $salesInvoice->invoice_date : '' }}</b></p>
                        <p><b>Customer Name: {{ isset($salesInvoice->customers->ledger_name) ? $salesInvoice->customers->ledger_name : '' }}</b></p>
                        <p><b>Sales Person: {{ isset($salesInvoice->sales_person->name) ? $salesInvoice->sales_person->name : '' }}</b></p>
                        <p><b>Branch Name: {!! (isset($salesInvoice->branch)) ? $salesInvoice->branch->branch_name : '-' !!}</b></p>
                        <p><b>Address: {!! (isset($salesInvoice->address)) ? $salesInvoice->address : '-' !!}</b></p>
                ...
                @if(isset($salesInvoice->items) && $salesInvoice->items->isNotEmpty())
                    @php
                        $i = 1;
                    @endphp
                    @foreach($salesInvoice->items as $key => $item)
                        <tr>
                            ...
                            <td>{{$salesInvoice->required_date}}</td>
                ...
                <tr>
                    @php
                    $total=((int)$item->net_amount -(int)$salesInvoice->items->get(0)->discount);
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">DISCOUNT</td>
                    <td align="right">{{ isset($salesInvoice->items->get(0)->discount_in_per) ? $salesInvoice->items->get(0)->discount_in_per : '0.00' }}%</td>
                    <td align="right">{{$salesInvoice->items->get(0)->discount}}</td>
                </tr>
                <tr>
                ...
                <tr>
                    @php
                    $sgst=((int)$salesInvoice->items->get(0)->tax/2);
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">SGST</td>
                    @foreach($state as $key=>$value)
                    @if($salesInvoice->state==$value->state)
                    ...
                    <td align="right">{{ isset($salesInvoice->sgst) ? $salesInvoice->sgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    @endif
                    @endforeach
                    <td align="right">{{ isset($salesInvoice->cgst) ? $salesInvoice->cgst : '0.00' }}</td>
                </tr>
                <tr>
                    ...
                    <td align="right">0%</td>
                    @else
                    <td align="right">{{$salesInvoice->items->get(0)->tax}}%</td>
                    @endif
                    @endforeach
                    
                    <td align="right">{{ isset($salesInvoice->igst) ? $salesInvoice->igst : '0.00' }}</td>
                </tr>
                <tr>
                    <td colspan="6" align="right">TOTAL AMOUNT</td>
                    <td align="right">{{$salesInvoice->total_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"></td>
                    <td colspan="4" align="right">FREIGHT</td>
                    <td align="right">{{$salesInvoice->total_other_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"><b>Credit Days: {{ isset($salesInvoice->credit_days) ? $salesInvoice->credit_days : 0 }} Days PDC</b></td>
                    <td colspan="4" align="right"><b>TOTAL GROSS AMOUNT (Rs.)</b></td>
                    <td align="right"><b>{{ isset($salesInvoice->grand_total) ? $salesInvoice->grand_total : '0.00' }}</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Delivery : at {{ isset($salesInvoice->target_warehouse->name) ? ' ' . $salesInvoice->target_warehouse->name : '' }}</b></p>
                        <p><b>Transpoter : {{ isset($salesInvoice->transporter) ? $salesInvoice->transporter : '' }}</b></p>
                        <p><b>Reference  : {{ isset($salesInvoice->reference) ? $salesInvoice->reference : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
