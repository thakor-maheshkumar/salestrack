...
<fieldset class="hidden-block scheduler-border">
    ...
    <div class="table-responsive">
        <table class="dynamic-table--warpper">
                ...
                ?>
                    <tr class="new-item">
                       <td>
                            ...
                            <select name="items[{{$element_counter}}][item_name]" data-plugin-selectTwo class="form-control item_name select2-elem select2" data-name="item_name" required="required">
                        ...
                        <td>
                            {{Form::select("items[$element_counter][unit]", [''=>'Select an Item']+$units, $item->unit ?? $item->uom, ['id'=>'unit','class'=>'form-control unit' ])}}
            ...
            @else
                <tr class="new-item f">
                    <td>
                        ...
                        <select name="items[{{$element_counter}}][item_name]" data-plugin-selectTwo class="form-control item_name select2-elem select2" data-name="item-name" required="required">
                            <option value="">Select Item Code/Name</option>
                            @if(!empty($stockItem))
                                @foreach($stockItem as $value)
            ...
            <tfoot>
                <tr>
                    @if(!isset($purchase_order_page))
                        <td colspan="3">
                            <a href="javascript:void(0);" class="btn btn-add add-item btn btn-success" data-toggle="tooltip" data-placement="bottom" title="Add New Item"><i class="fa fa-plus"></i></a>
                        </td>
                    @endif
                </tr>
            </tfoot>
        ...
        $(document).ready(function(){
            ...
            $(document).on('click', '.add-item', function() {
                ...
                if (! has_error) {
                    ...
                    var temp_counter = $(document).find(".add-items-list").children("tr.new-item").length;
            ...
            getAmount = function(rate,quantity,discount,index)
            {
                ...
                var discount_amount = $("input[name='items["+index+"][discount_amount]']").val();
                ...
                {
                    discount_amount = ((discount_in_per*net_amount)/100);
                }else{
                   discount_amount=0; 
                }
                ...
                if(discount_amount != 0)
                {
                    ...
                    $("input[name='items["+index+"][discount_amount]']").val('0'); 
                }
            };
            ...
            $(document).on('focusout', '.tax', function(e) {
                var $tr = $(this).closest('tr');
                var index = $tr.index();
                countAfterFocusOut(index);
            });
            ...
            getMaterialDetails = function(material_id)
            {
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                $.ajax({
                    type:'POST',
                    url:'{{ route("purchase.material-item-details") }}',
                    data:{ material_id:material_id },
                    success:function(data){
                        //var item_id = $("input[name='items["+index+"][item_id]']").val();
                        if(data.success)
                        {
                            $('.add-items-list').html(data.view);
                            $(".add-items-list .select2-elem").select2();
                        }
                    }
                });
            };
            ...
            $(document).on('change', '#material_id', function(e) {
                ...
                if(material_id)
                {
                    getMaterialDetails(material_id);
                }
            });
        });
    </script>
