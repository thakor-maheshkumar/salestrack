    ...
    <div class="form-row">
        <div class="col-md-12">
            ...
            {!! Form::label('branch_id', 'Branch') !!}
                {!! Form::select('branch_id', (!empty($consignee_address)) ? [$order->main_branch => $order->main_branch]+$consignee_address : [$order->main_branch => $order->main_branch], old('branch_id', isset($order->main_branch) ? $order->main_branch : ''), ['class' => 'form-control branch_id','id'=>'branch_id'] ) !!}
            @else
                {!! Form::label('branch_id', 'Branch') !!}
                {!! Form::select('branch_id', (!empty($consignee_address)) ? ['' => $order->main_branch]+$consignee_address : [$order->main_branch => $order->main_branch], old('branch_id', isset($order->main_branch) ? $order->main_branch : ''), ['class' => 'form-control branch_id','id'=>'branch_id'] ) !!}
            @endif
            @else
            {!! Form::label('branch_id', 'Branch') !!}
        ...
        <!-- new code Here 13-10-2021 -->
            <input type="text" name="branch_id" class="form-control mainBranch" id="mainBranch" style="display:none" disabled="true">
        <!-- -->
        </div>
        ...
        if (supplier_id != '')
        {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.supplier_details.consignee_addresses !=null){
