...
<div class="table-responsive">
    <table class="dynamic-table--warpper table table-bordered">
        <thead>
            <th>Item</th>
            <th>Quantity</th>
            <th></th>
        ...
        <tbody class="add-items-list">
            @if(isset($bom->bom_items) && !empty($bom->bom_items))
                @foreach($bom->bom_items as $sKey => $bom_item)
                    <tr class="new-item">
                        <td>
                            {!! Form::select("bom_stock_items[$element_counter][item_item_id]", [" "=>"Select a Items"] +$stock_items, $bom_item['stock_item_id'] , ['class' => 'form-control item_item_id select2-elem select2', 'id' => 'item_item_id','style'=>'width:300px']) !!}
                        </td>
            ...
            @else
                <tr class="new-item">
                    <td>
                        {!! Form::select("bom_stock_items[$element_counter][item_item_id]",['' => 'Select a Items'] + $stock_items, old("item_item_id[$element_counter]"), ['class' => 'form-control item_item_id select2-elem select2', 'id' => 'item_item_id','style'=>'width:300px']) !!}
                    </td>
                    <td class="datepicker-box">
