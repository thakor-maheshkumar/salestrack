                    ...
                    <div class="card-header">
                        <div class="row align-items-center">
                            ...
                            <div class="col-12 text-right">
                                ...
                                @if(\Helper::userHasPageAccess('bom.create'))
                                    <a href="{{ route('bom.create') }}" class="btn btn-success">Create BOM</a>
                                @endif
                            </div>
                        </div>
                    </div>
                            ...
                            <table id="group_table" class="table datatable table-condensed">
                                <thead>
                                    <tr>
                                        ...
                                        <th>BOM Name</th>
                                        <th>Item Name</th>
                                        <th style="display: none;">No Of Unit</th>
                                        <th style="display:none">Rate Of Material</th>
                                ...
                                @if(isset($bom) && count($bom) > 0)
                                    @foreach($bom as $key => $item)
                                        <tr>
                                            ...
                                            <td style="display:none;">{{$item->no_of_unit}}</td>
                                            <td style="display:none;">{{$item-> rate_of_material }}</td>
                                            <td style="display:none;">{{$item->active}}</td>
                                            <td class="action--cell">
                                                <ul class="action--links">
                                                    @if(\Helper::userHasPageAccess('bom.edit'))
                                                        <li>
                                                            <a href="{{ route('bom.edit', $item->id) }}" class="btn btn-info"><i class="fas fa-edit"></i></a>
                                                        </li>
                                                        <li>
                                                            {!! Form::open(['url' => route('bom.destroy', $item->id), 'method' => 'DELETE']) !!}
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="5" align="center">BOM not found...</td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
