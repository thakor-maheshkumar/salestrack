...
<div class="form-group">
    ...
    {!! Form::label('branch_id', 'Branch') !!}
            {!! Form::select('branch_id', ['' => 'Select a Branch'] + $consignee_address, old('branch_id', isset($order->supplier_id) ? $order->branch_id : ''), ['class' => 'form-control branch_id', (count($consignee_address) == 0) ? 'disabled' : ''] ) !!}
...
//                console.log(po_id);
                ...
                $.ajax({
                    ...
                    success:function(data){
                            ...
                            var textjk=data.po_order_details.branch_id;
                            $("#warehouse_id").val(data.po_order_details.warehouse_id);
                           var branch_id=data.po_order_details.supplier_id;
                            $.ajax({
                                type:"POST",
                                ...
                                success:function(data){
                                    //console.log(data);
                                    var mk=data.supplier_details;
                                    console.log(mk);
                                    //$('#branch_id').html(options);
                                       var options='';
                                        $.each(mk, function(key, obj) {
                                        options += '<option value="'+obj.branch_name+'">' +obj.branch_name+ '</option>';   
                                       });
                                        $('.branch_id').html(options);
                                        //$(".branch_id").val(textjk);
                                }
                            })
