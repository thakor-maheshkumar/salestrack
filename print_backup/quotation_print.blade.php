...
<body>
    <section class="pdf-block">
        ...
        <table border="1" class="main-table" style="width:705px">
            <tbody>
        ...
        <table border="1" class="main-table" style="width:705px">
            <tbody>
                ...
                <tr>
                    <td colspan="7" align="center"><b>Quotation</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b> Quotation NO :  {{ isset($quotation->quotation_no) ? $quotation->quotation_no : '' }}</b></p>
                        <p><b>Valid Date: {{ isset($quotation->valid_till) ? $quotation->valid_till : '' }}</b></p>
                        <p><b>Branch Name: {!! (isset($quotation->branch)) ? $quotation->branch->branch_name : '-' !!}</b></p>
                        <p><b>Sales Person: {{ isset($quotation->salesPerson->name) ? $quotation->salesPerson->name : '' }}</b></p>
                        <p><b>Address: {{ isset($quotation->address) ? $quotation->address : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
                ...
                @if(isset($quotation->items) && $quotation->items->isNotEmpty())
                    ...
                    @foreach($quotation->items as $key => $item)
                        <tr>
                            ...
                            <td align="right">{{ isset($item->total_amount) ? $item->total_amount : '0.00' }}</td>
                        </tr>
                        @php
                            $i++;
                        @endphp
                ...
                <tr>
                 ...
                 <tr>
                    @php
                    $total=((int)$quotation->total_net_amount - (int)$quotation->items->get(0)->discount);
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">DISCOUNT</td>
                    <td align="right">{{ isset($quotation->items->get(0)->discount_in_per) ? $quotation->items->get(0)->discount_in_per : '0.00' }}%</td>
                    <td align="right">{{$quotation->items->get(0)->discount}}</td>
                </tr>
                <tr>
                ...
                <tr>
                    @php
                    $sgst=((int)$quotation->items->get(0)->tax/2)
                    @endphp
                    <td colspan="3" align="right" class="no-border-b">SGST</td>
                    <td align="right">{{$sgst}}%</td>
                    <td align="right">{{ isset($quotation->sgst) ? $quotation->sgst : '0.00' }}</td>
                </tr>
                <tr>
                    <td colspan="3" align="right" class="no-border-b no-border-t">CGST</td>
                    <td align="right">{{$sgst}}%</td>
                    <td align="right">{{ isset($quotation->cgst) ? $quotation->cgst : '0.00' }}</td>
                </tr>
                <tr>
                    <td colspan="3" align="right" class="no-border-t">IGST</td>
                    <td align="right">{{$quotation->items->get(0)->tax}}%</td>
                    <td align="right">{{ isset($quotation->igst) ? $quotation->igst : '0.00' }}.00</td>
                </tr>
                <tr>
                    <td colspan="6" align="right">TOTAL AMOUNT</td>
                    <td align="right">{{$quotation->total_net_amount}}</td>
                </tr>
                <tr>
                    <td colspan="2"></td>
                    <td colspan="4" align="right">FREIGHT</td>
                    <td align="right">{{ isset($quotation->total_other_net_amount) ? $quotation->total_other_net_amount : '0.00' }}</td>
                </tr>
                <tr>
                    <td colspan="2"><b>Credit Days: {{ isset($quotation->credit_days) ? $quotation->credit_days : 0 }} Days PDC</b></td>
                    <td colspan="4" align="right"><b>TOTAL GROSS AMOUNT (Rs.)</b></td>
                    <td align="right"><b>{{ isset($quotation->grand_total) ? $quotation->grand_total : '0.00' }}</b></td>
                </tr>
                <tr>
                    <td colspan="7">
                        <p><b>Delivery : at {{ isset($quotation->target_warehouse->name) ? ' ' . $quotation->target_warehouse->name : '' }}</b></p>
                        <p><b>Transpoter : {{ isset($quotation->transporter) ? $quotation->transporter : '' }}</b></p>
                        <p><b>Reference  : {{ isset($quotation->reference) ? $quotation->reference : '' }}</b></p>
                    </td>
                </tr>
                <tr>
                    <td colspan="7">
